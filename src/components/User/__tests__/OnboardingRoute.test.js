...
// ── Nothing renders between the URL and the resolved screen (ONB-024) ────────

describe('no interstitial ahead of the journey', () => {
  it('lands a first-time user straight on Welcome', async () => {
    renderJourney()

    await atScreen(t('onboarding.welcome.title'))
    // The retired carousel wrote this key on its way out (ADR-024). Nothing
    // reads or writes it any more, and nothing stands ahead of the journey.
    expect(window.localStorage.getItem('nowry_onboarding_teaser_seen')).toBeNull()
  })

  it('resolves an already-activated user Home with no screen in between', async () => {
    server.journey = { ...server.journey, status: 'activated', activated_at: '2026-08-14T00:00:00Z', resume_screen: null }
