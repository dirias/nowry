...
/** A platform with no voice enumeration never calls back, and never unsubscribes. */
const noVoices = () => () => {}

const useDeckSettings = ({ open, deckId, initialSection = 'study', onSaved, onClose, subscribeToVoices = noVoices }) => {
  ...
  /*
   * The voice list comes from the client (MOB-004). The browser's
   * `speechSynthesis` has no React Native counterpart, and enumerating voices is
   * exactly the kind of thing each platform does its own way, so the caller
   * subscribes and this hook only holds the result. `subscribeToVoices` takes a
   * callback and returns its unsubscribe, so a platform with no voices at all
   * simply never calls back.
   */
  useEffect(() => subscribeToVoices(setAvailableVoices), [subscribeToVoices])
