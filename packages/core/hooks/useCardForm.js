...
import { extractDeckId } from '../utils/formUtils'
import { cardToFormState, contentPredicatesFor, emptyCardValues, specFor } from '../domain/cardTypes'
...
const useCardForm = ({ open, card = null, initialSection = null, onSaved, onClose, focusTarget = noFocus }) => {
  ...
  /*
   * The Cmd/Ctrl+Enter shortcut used to live here as a `document` listener.
   * There is no `document` in @nowry/core and no hardware keyboard to assume on
   * a phone, so the hook exposes `primaryAction` and the caller binds whatever
   * gesture its platform has. `CreateCardModal` binds the keystroke.
   */
