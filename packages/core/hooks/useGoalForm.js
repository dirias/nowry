...
import { blankMilestone } from '../domain/goalDerivation'
import { describeApiError, emptyToNull } from '../utils/formUtils'
...
/**
 * Moving focus is the client's job, not this hook's (MOB-003B, MOB-004).
 * `focusFirstControl` reaches into DOM nodes, and this module is shared, so the
 * caller supplies it. The web app passes the DOM implementation; mobile will
 * pass something that calls `.focus()` on a ref. The default is a no-op.
 */
const noFocus = () => {}

const useGoalForm = ({ open, goal, focusAreaId, initialSection = null, focusControl = noFocus }) => {
  ...
  const focusGroup = useCallback((name) => focusControl(nodes.current[name]), [focusControl])
