/**
 * Pure helpers shared by every form surface, on either client. No JSX, no Joy,
 * no t() — this module returns values and translation *keys*, never rendered
 * strings, and it touches no DOM.
 *
 ...
 * used to render as a bare "AxiosError" or, on the visual card, as nothing.
 *
 * The scroll and focus helpers that used to sit beside these stayed behind in
 * `src/components/Common/Form/formUtils.js` (MOB-003B). They operate on DOM
 * nodes, so they are the web client's, not the shared layer's.
 */
